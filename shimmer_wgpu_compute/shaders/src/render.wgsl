...
fn render(@builtin(global_invocation_id) global_id: vec3u) {
    ...
    for (var i: u32 = 0; i < arrayLength(&triangles); i += 1u) {
        ...
        var bary_coords = triangle_barycentric_point_coords(triangle, vram_coords);
        var is_inside = (bary_coords.x >= 0.0) && (bary_coords.y >= 0.0) && (bary_coords.x + bary_coords.y) < 1.0;

        if is_inside {
            var rgba_norm = rgba8_normalize(triangle.vertices[0].color);
